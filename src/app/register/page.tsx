  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (loginRes.ok) {
        ...
        router.refresh(); // Force a hard refresh to apply cookies in middleware
      } else {
        router.push('/login');
      }
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-[#fafafa]">
      <div className="w-full max-w-[420px] bg-white border border-[#e8e8e8] rounded-[12px] p-11 shadow-sm">
        ...
        <h1 className="text-[24px] font-extrabold text-black mb-1.5">Create account</h1>
        <p className="text-[14px] text-[#555] mb-8">Start shortening links in seconds. Free forever.</p>
        ...
        <p className="text-[13px] text-[#555] text-center mt-6">
          Already have an account? <Link href="/login" className="text-black font-semibold hover:text-[#2233ee]">Log in</Link>
        </p>

        <p className="text-[12px] text-[#999] text-center mt-4 leading-[1.6]">
          By signing up you agree to our <a href="#" className="text-[#555] underline">Terms</a> and <a href="#" className="text-[#555] underline">Privacy Policy</a>.
        </p>
      </div>
    </div>
  );
